...
public interface AuditTrailRepoisitory extends JpaRepository<AuditTrail, Long> {
    ...
    @Query("SELECT new com.kurt.gym.core.persistence.entity.AuditTrail(e.id, e.action, e.message, e.createdAt) from AuditTrail e order by e.createdAt desc")
    Page<AuditTrail> findAuditTrailOrderByCreatedAtDesc(String search, Pageable pageable);

    @Query("SELECT new com.kurt.gym.core.persistence.entity.AuditTrail(e.id, e.action, e.message, e.createdAt) from AuditTrail e where (e.createdAt between ?1 and ?2) or e.createdAt = ?1 order by e.createdAt desc")
